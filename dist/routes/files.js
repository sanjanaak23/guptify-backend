    ...
    try {
        ...
        if (type) {
            if (type === 'image') {
                ...
                queryBuilder = queryBuilder.or('type.ilike.%word%,type.ilike.%excel%,type.ilike.%powerpoint%,type.ilike.%officedocument%');
            }
            else if (type === 'video') {
        ...
        if (sizeMin) {
            const minBytes = parseInt(sizeMin) * 1024 * 1024;
            queryBuilder = queryBuilder.gte('size', minBytes);
        }
        if (sizeMax) {
            const maxBytes = parseInt(sizeMax) * 1024 * 1024;
...
router.post('/:id/share', authenticate, async (req, res) => {
    try {
        ...
        const { data: shareData, error: shareError } = await supabase_1.supabase
            .from('file_shares')
            .insert([{
                file_id: id,
...
router.delete('/trash/empty', authenticate, async (req, res) => {
    try {
        ...
        const { data: files, error: fetchError } = await supabase_1.supabase
            .from('files')
            .select('*')
            .eq('user_id', req.user.id)
            .eq('is_deleted', true);
...
router.get('/:id/preview', authenticate, async (req, res) => {
    try {
        ...
        const { data: previewData, error: previewError } = await supabase_1.supabase.storage
            .from('files')
